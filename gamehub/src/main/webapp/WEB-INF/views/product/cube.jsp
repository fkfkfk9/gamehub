...
<head>
<meta charset="UTF-8">
<link rel="stylesheet" type="text/css" href="/css/product/cube.css" />
...
<body>
						...
						<c:forEach items="${pvlist}" var="list" >
							<c:if test="${idx % 3 == 0}">
								...
								<div class="tdwarp">
									...
									<img width="100%" height="130px" src="/admin/product/displayFile?fileName=${list.pd_img }"></img></a>
									<div class="titlediv">
									<a href="/product/read?pd_num=${list.pd_num}">
									<span>[${list.cg_p_name}]</span>
									<span>${list.pd_name}</span></a></div>
									<div class="textdiv"><span class="tagspan">${list.cg_name}</span>
									<span class="tagspan"><c:out value="${list.pd_korea eq 'Y'?'한국어':'영어'}"/></span></div>
									<div class="textdiv"><span class="tagspan">${list.pd_publisher}</span></div>
									<div class="textdiv"><p>정상가 : <del><fmt:formatNumber type="number" maxFractionDigits="3" value="${list.pd_price}"/></del></p></div>
									<div class="textdiv">
										<c:set value="${(list.pd_price/100)*(100-list.pd_discount) }" var="price" />
										<p>판매가 : <fmt:formatNumber type="number" maxFractionDigits="3" value="${price}"/></p>
									</div>
									<div class="amount">
									...
									<div class="btndiv">
										<span class="discount">${list.pd_discount}%</span>
										<input type="button" class="cartBtn" data-num="${list.pd_num }"  data-amount="1" value="장바구니 담기">
									</div>
								</div><!-- tdwarp END -->
								</td>
	...
	<script src="/js/product/cube.js"></script>
	<script>
	$(function() {
